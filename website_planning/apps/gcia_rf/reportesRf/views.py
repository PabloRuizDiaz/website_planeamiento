...
def reportes(request):
    dash = reporteRFModel.objects.filter(publicar=True).order_by('-id')

    return render(request, 'gcia_rf/reportes.html', {"dash": dash})
...
def cargaReporte(request, id_rep):
    if request.method == 'GET':
        reporte = reporteRFModel.objects.get(id=id_rep)
