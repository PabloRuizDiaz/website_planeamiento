...
def reportes(request):
    dash = reporteTxModel.objects.filter(publicar=True).order_by('-id')

    return render(request, 'gcia_tx/reportes.html', {"dash": dash})
...
def cargaReporte(request, id_rep):
    if request.method == 'GET':
        reporte = reporteTxModel.objects.get(id=id_rep)
