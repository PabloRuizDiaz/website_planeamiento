...
def reportesMat(request):
    dash = reporteMatModel.objects.filter(publicar=True).order_by('-id')

    return render(request, 'gcia_plan/materiales/reportes/reportes.html', {"dash": dash})
...
def cargaReporte(request, id_rep):
    if request.method == 'GET':
        reporte = reporteMatModel.objects.get(id=id_rep)
