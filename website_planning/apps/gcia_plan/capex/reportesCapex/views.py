...
def reportes(request):
    dash = reporteCapexModel.objects.filter(publicar=True).order_by('-id')

    return render(request, 'gcia_plan/capex/reportes/reportes.html', {"dash": dash})
...
def cargaReporte(request, id_rep):
    if request.method == 'GET':
        reporte = reporteCapexModel.objects.get(id=id_rep)
