...
def reportes(request):
    dash = reporteRedModel.objects.filter(publicar=True).order_by('-id')

    return render(request, 'gcia_plan/red/reportes/reportes.html', {"dash": dash})
...
def cargaReporte(request, id_rep):
    if request.method == 'GET':
        reporte = reporteRedModel.objects.get(id=id_rep)
