...
def reportes(request):
    dash = reporteServFijModel.objects.filter(publicar=True).order_by('-id')

    return render(request, 'gcia_servfij/reportes.html', {"dash": dash})
...
def cargaReporte(request, id_rep):
    if request.method == 'GET':
        reporte = reporteServFijModel.objects.get(id=id_rep)
