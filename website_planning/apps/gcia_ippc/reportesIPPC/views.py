...
def reportes(request):
    dash = reporteIPPCModel.objects.filter(publicar=True).order_by('-id')

    return render(request, 'gcia_ippc/reportes.html', {"dash": dash})
...
def cargaReporte(request, id_rep):
    if request.method == 'GET':
        reporte = reporteIPPCModel.objects.get(id=id_rep)
